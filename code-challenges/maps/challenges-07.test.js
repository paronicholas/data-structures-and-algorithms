...
const forEachTwoToThe = (arr) => {
  const res = [];
  arr.forEach(value => res.push(2**value));
  return res;
};
...
const mapTwoToThe = (arr) => {
  const res = arr.map(value => 2 ** value);
  return res;
};
...
const charCode = (arr) => {
  const res = arr.map((value, index) => arr[index].charCodeAt());
  return res;
};
...
const extractAbilities = (arr) => {
  const res = arr.map(value => value.ability.name);
  return res;
};
...
const extractStats = (arr) => {
  ...
  Object.values(arr).map(value => {
    const Attacks = new Object();
    Attacks.name = value.stat.name;
    Attacks.total = value.effort + value.baseStat;
    res.push(Attacks);
  })
  return res;
};
